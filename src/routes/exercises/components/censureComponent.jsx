...
import '../../../styles/censureComponent.css'
...
export default function CensureComponent ({ data, returnScore, isFinished }) {
  ...
  return (
    <div className="binary-test-ex">
      ...
      {!nextQuestion && <button className="button-play" onClick={() => { setNextQuestion(true) }}>{data.btnAnswer}</button>}
      {nextQuestion && <button className="button-play" onClick={handleClick}>{data.btnNext}</button>}
    </div>
  )
}
