...
import DroppableComponent2 from './droppableComponent2'
import '../../../styles/dndComponent2.css'

export default function DndComponent2 ({ data, returnScore, reset }) {
  ...
  return (
    <DragDropContext onDragEnd={onDragEnd}>
      <div className='drag-drop-context'>
        {
                    !myData.isSingle
                      ? <DroppableComponent2 column={myData.columns['column-2']} options={myData.columns['column-2'].optionsIds.map(optionId => myData.options[optionId])} returnData={setOptionsData} res={res} />
                      : <>
                        <DroppableComponent2 column={myData.columns['column-1']} options={myData.columns['column-1'].optionsIds.map(optionId => myData.options[optionId])} res={res} />
                        <DroppableComponent2 column={myData.columns['column-2']} options={myData.columns['column-2'].optionsIds.map(optionId => myData.options[optionId])} returnData={setOptionsData} isLoop={myData.isLoop} res={res} />
                        </>
                }
      </div>
    </DragDropContext>
