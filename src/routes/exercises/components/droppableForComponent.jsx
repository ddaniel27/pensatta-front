...
export default function DroppableForComponent ({ column, options, returnData }) {
  ...
  const { t } = useTranslation('droppableForComponent')
  ...
  return (
    <div className={`droppable-container ${column.id}-loop`}>
      ...
      {column.id === 'column-2' && <span className="block-top">{t('repeat')} <input type="text" onChange={handleChange} maxLength="2" name="forBlock" value={value}/> {t('times')}</span>}
