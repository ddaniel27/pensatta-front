...
export default function Ex28 () {
  ...
  async function animate (peluche, animationSequence) {
    let segments = []
    if (animationSequence === 'Saludar') { segments = [0, 120] } else if (animationSequence === 'No') { segments = [120, 192] } else if (animationSequence === 'Si') { segments = [192, 264] } else if (animationSequence === 'Selfie') { segments = [264, 360] } else if (animationSequence === 'Saltar') { segments = [360, 432] } else if (animationSequence === 'Reir') { segments = [432, 504] } else if (animationSequence === 'Sorpresa') { segments = [504, 576] } else if (animationSequence === 'Llorar') { segments = [576, 648] } else if (animationSequence === 'Celular') { segments = [648, 792] } else if (animationSequence === 'Bailar') { segments = [792, 984] }
    for (let i = 0; i < 30; i++) {
        ...
        (setScore, setPhase) => (
          <div className="animation-ex28">
            <div className="animation-related">
              ...
              {!playAnimation && <p onClick={handlePlay}>{myData.btnPlay}</p>}
            </div>
